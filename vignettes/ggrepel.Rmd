...
|`segment.curvature` | `0`      | line segment curve
|`segment.angle` | `90`         | line segment curve angle
|`segment.npc`   | `1`          | line segment curve control points
...
The line segments can also be curved, as in `geom_curve()`. Use `segment.curvature` to control the curve, increase `segment.ncp` for a smoother curve, and use `segment.angle` to control the skew of the curve.
...
Setting the curvature very low will also allow an angle in the line segment.
