...
ggrepel will show a warning if some data points could not be labeled
...
entire session. This option can be overridden by providing the 
`max.overlaps` argument.

```{r show_all_labels, echo=TRUE, fig.width=6, fig.height=3, warning=TRUE}
set.seed(42)

dat4 <- data.frame(
  x = rep(1, length.out = 12),
  y = rep(1, length.out = 12),
  label = letters[1:12]
)
...
p2 <- ggplot(dat4, aes(x, y, label = label)) +
  geom_point() +
  geom_label_repel(box.padding = 0.5, max.overlaps = 1)
