...
The following aesthetics are the same for all functions (`geom_text()`, `geom_text_repel()`, etc.):
...
These aesthetics are specific to ggrepel functions, but not applicable to ggplot2 functions:
...
And the following aesthetics are specific to `geom_marquee_repel()`:

|Aesthetic       | Default      | Description
|--------------- | ------------ | ------------------------------------------------
|`style` | `NULL`    | a `marquee_style` object specifying the styling, see `marquee::style()`
|`width` | `NA`    | a `grid::unit()` object specifying the width of each marquee element, e.g., `unit(2, "cm")`


## Advanced examples

The ggrepel package works well with the [ggpp] package!

For more advanced ggrepel examples, check out the [ggpp examples] by
...
The [ggpp examples] page describes how to use many advanced functions,
including:
...
Use the `arrow.fill` aesthetic to set the fill color for closed arrow heads
independently from `segment.colour`. By default, `arrow.fill` matches the
segment color.

```{r arrow_fill, echo=TRUE, fig.width=9, fig.height=4}
df <- mtcars[1:8,]
df$car <- rownames(df)
...
Use `linetype` and `linewidth` to customize the border of `geom_label_repel()`.
Set `linewidth = 0` to hide the border entirely.

```{r label_border, echo=TRUE, fig.width=9, fig.height=4}
df <- mtcars[1:8,]
df$car <- rownames(df)
...
Use `lineheight` to control the spacing between lines in multi-line labels.

```{r label_lineheight, echo=TRUE, fig.width=12, fig.height=4}
df <- data.frame(
  x = c(1, 2, 3),
  y = c(1, 2, 3),
  label = c("Compact\nline\nspacing", "Default\nline\nspacing", "Spacious\nline\nspacing")
...
ggrepel now works with the [marquee] package!

See the [marquee documentation] for more information about how to control the
styles.

We can use the function `geom_marquee_repel()` to plot rich text and images with
automatic positioning, as shown in this example below:

[marquee]: https://github.com/r-lib/marquee/
[marquee documentation]: https://marquee.r-lib.org/
...
<div class="alert alert-info" role="alert">
  **Note:** When using `geom_marquee_repel()`, you may want to use **[ragg]**
  instead of the default renderer. This can improve the display of your plot.
</div>

[ragg]: https://ragg.r-lib.org/articles/ragg_quality.html

If we use `geom_marquee_repel()` with `dev="png"` instead of `dev="ragg_png"`,
then the result is more pixelated:
...
To learn more, see the [ragg] documentation and read about the `device`
argument for the `ggplot2::ggsave()` function.

### Repel axis labels with element_text_repel()

Use `element_text_repel()` as a theme element to repel crowded axis labels.
This is especially useful when you have many categories on a categorical axis.

```{r element_text_repel_basic, echo=TRUE, fig.width=6, fig.height=8}
...
You can customize the connecting segments with curvature and color options:

```{r element_text_repel_curved, echo=TRUE, fig.width=6, fig.height=8}
p + coord_cartesian(ylim = c(-32, 64)) +
  theme(
...
```{r element_text_repel_secondary, echo=TRUE, fig.width=7, fig.height=7}
x <- c(0, 4.9, 5, 5.1, 10)
...
```{r timing, echo = TRUE, message=TRUE, fig.width=7, fig.height=5}
p <- ggplot(mtcars,
